...
function createEventActions(ipcRenderer) {
  ...
  const onTriggerScreenshot = createEventListener(ipcRenderer, {
    channel: 'trigger-screenshot',
    label: 'onTriggerScreenshot'
  });
  ...
  const onMemoryReviewQueueUpdated = createEventListener(ipcRenderer, {
    channel: 'memory-review-queue-updated',
    label: 'onMemoryReviewQueueUpdated'
  });
