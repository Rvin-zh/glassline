export function createChatUiManager({
    ...
    function addChatMessage(type, content, options = {}) {
        ...
        if (record.screenshotId) {
            messageDiv.dataset.screenshotId = record.screenshotId;
        }
    ...
    async function submitManualContextMessage() {
        if (!chatManualInput) {
            return;
        }
        ...
        const message = addChatMessage('voice-mic', text);
        addMonitorLog?.('info', 'manual-message-sent', 'Manual message sent', 'mic', {
            chars: text.length
        });
        ...
        if (typeof onManualMessageSubmitted === 'function') {
            await onManualMessageSubmitted({ text, message });
        } else {
            showFeedback?.('Message added to context', 'success');
        }
