export function setupIpcListeners({
    ...
    windowApi.onScreenshotTakenStealth((count) => {
        ...
        removeScreenshotMessagesByIds?.(payload.replacedScreenshotIds);
        setScreenshotsCount(Number(payload.count || 0));
        updateUi();
        addChatMessage(
            'screenshot',
            payload.origin === 'auto' ? 'Auto screenshot captured' : 'Screenshot captured',
            {
    ...
    if (windowApi.onTriggerScreenshot) {
        windowApi.onTriggerScreenshot(() => {
            addMonitorLog('info', 'shortcut-event', 'Global Screenshot shortcut triggered');
            takeStealthScreenshot().catch((error) => {
                console.error('Global Screenshot trigger failed:', error);
                addMonitorLog('error', 'shortcut-screenshot-failed', error.message);
            });
        });
    }
