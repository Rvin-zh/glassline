export function setupEventListeners({
    ...
    if (autoScreenToggle) {
        autoScreenToggle.addEventListener('click', () => {
            toggleAutoScreen().catch((error) => {
                console.error('Failed to toggle Auto Screen:', error);
            });
        });
    }
    ...
    document.addEventListener('keydown', (event) => {
        ...
        if (isShortcutPressed?.(event, 'takeScreenshot')) {
            event.preventDefault();
            addMonitorLog('info', 'shortcut-local', 'Local Screenshot shortcut captured; awaiting global Screenshot event');
            return;
        }
